...
class GASearchThreadPool(Thread):
    ...
    """
    def __init__(self, gservice=None, user=None, groups=None, check_is_member_func=None, groups_user_in=None):
        Thread.__init__(self)
        self.gservice = gservice
        self.user = user
        self.groups = groups
        self.check_is_member_func = check_is_member_func
        self.groups_user_in = groups_user_in
        ...
        for group in self.groups:
            find_thread = Thread(target=self.check_is_member_func,
                                 args=(self.gservice,# Service module, used as bridge
                                       ...
                                       self.groups_user_in, # list of all groups which user is in.
                                       )
                                )
            find_thread.start()
...
class GASearch(GABase):
    ...
    def search(self):
        ...
        elif self.user is None and self.group is None:
